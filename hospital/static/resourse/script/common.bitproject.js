...
function fillTable(tableid,obs,modifyFunction=null,deletefunction=null,printfunction=null){
    ...
    for(var i=0; i<obs.length; i++){
        ...
        if(hasButton){
            ...
            buttonup.setAttribute('data-target','#staticBackdrop');
...
function fillCombo3(combo, message, list, attribute,attribute1, selectedvalue) {
    ...
    for (index in list) {
        var option = document.createElement("option");
        option.value = JSON.stringify(list[index]);
        var text = document.createTextNode(list[index][attribute] +" ("+list[index][attribute1]+")" );
...
function comboBoxBinder(field,pattern,obj,prop,oldobj, nullable=false){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
    ...
    if (!nullable){
        ...
        if(oldob != null && (oldob[prop] != null ||  ob[prop] != null))
            field.style.border = updated;
        else
            field.style.border = valid;
    }
...
function fileChooser(parent,id,lblText,labelWidth,extenstions,maxsize,required,ob,prop,oldob,prop2='photoname'){
    ...
    field.setAttribute('data-property2',prop2);
...
function drawFile(field){
    ...
    var property2 = field.getAttribute('data-property2');
...
function fillInnerTable(tableId, obs, modifyFunction = null, deleteFunction = null) {
    ...
    for (var i = 0; i < obs.length; i++) {
        ...
        if (hasButton) {
            ...
            buttonInnerUpdate.setAttribute('data-toggle','tooltip');
            buttonInnerUpdate.setAttribute('data-placement','bottom');
            buttonInnerUpdate.setAttribute('title','Edit');
            ...
            buttonInnerDelete.setAttribute('data-toggle','tooltip');
            buttonInnerDelete.setAttribute('data-placement','bottom');
            buttonInnerDelete.setAttribute('title','Remove');
            buttonInnerDelete.innerHTML = '<i class="fa fa-recycle"></i> ';
